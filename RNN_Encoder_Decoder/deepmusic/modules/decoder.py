...
class Lstm(DecoderNetwork):
    ...
    def build(self):
        ...
        # RNN network
        def get_a_cell(lstm_size, input_prob,keep_prob):
            lstm = tf.nn.rnn_cell.LSTMCell(lstm_size)
            drop = tf.nn.rnn_cell.DropoutWrapper(lstm, input_keep_prob=input_prob,output_keep_prob=keep_prob)
            return drop
        ...
        [get_a_cell(self.args.hidden_size, 1,0.9) for _ in range(self.args.num_layers)]
        )
